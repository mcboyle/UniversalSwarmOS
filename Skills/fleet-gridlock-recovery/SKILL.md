...
**Diagnosis:** The queue is starved because ghost rows (from agents that crashed or timed out) are stuck in `STARTED` state, artificially inflating the active count beyond `BD_DISPATCH_MAX_ACTIVE`.
**Resolution:**
1. Check for stale locks: `rm -f /tmp/bd-tmux-dispatch.lock`
2. Purge the ghost rows back to `aborted`:
   ```bash
   sed -i 's/started/aborted/g' /home/mboyle/bd-persist/DISPATCH-LEDGER.tsv
   ```
3. Dynamically bump the queue limit to allow the swarm to feast:
   ```bash
   BD_DISPATCH_MAX_ACTIVE=100 bash /home/mboyle/bd-persist/harness/bd-tmux-dispatch.sh
   ```

## 2. Linux Tripwire vs. Agent Policy

**Symptom:** Agent commands are repeatedly killed by `bd-tripwire-hook.py` despite having `-MCB` (Operator Executive Order) explicitly in their prompt.
...
```bash
BD_TRIPWIRE_OK="Authorized by -MCB" pytest tests/a.py tests/b.py
```
...
3. Once validated, forcefully land the PR bypassing GitHub checks entirely:
   ```bash
   BD_AUTO_MERGE_SKIP_CI=1 bash /home/mboyle/bd-persist/harness/bd-auto-merge.sh
   ```

## 5. Codex Agent Visibility

**Symptom:** Codex agents launched via `bd-launch-role.sh` do not appear in the operator's UI.
**Resolution:**
By default, they launch headlessly. To force them to route through the app server and appear in the UI, they must be executed with `BD_CX_RC=1`:
```bash
BD_CX_RC=1 bash /home/mboyle/bd-persist/harness/bd-launch-role.sh worker codex <name>
```
...
**Resolution:**
Use the `bd-say.sh` tool with the `DOWN` or `IMMEDIATE` keyword to instantly bypass the batch queue and force an `[BYPASS] Immediate escalation` interrupt directly into the PM's terminal:
```bash
/home/mboyle/bd-say.sh bd-pm-B "NEXT: HALT (-MCB). IMMEDIATE."
```
