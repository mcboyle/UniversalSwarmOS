...
- Route trivial formatting or regex sub-tasks to local Ollama/LiteLLM satellite models via `fleet-satellite-orchestration`.
...
- **Telemetry Guard Enforcement**: The standalone `swarm-telemetry-guard.py` daemon actively polices the `usage.sqlite` database in 10-minute intervals. 
