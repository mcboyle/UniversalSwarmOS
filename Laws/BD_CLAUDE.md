# CLAUDE.md — operating contract for ${TARGET_REPO_PATH}

Read fully before editing. Sole agent-facing contract for ${TARGET_REPO_PATH}: standing authority, safety boundaries, required commands, links to focused owners. Incidents live in Git and `CHANGELOG.md`, not here.

## A1 | Authority and scope

${TARGET_REPO_PATH} (BD): self-hosted Flask, Playwright, React/TypeScript batch downloader, operated by Matthew. Authoritative repo: `${TARGET_REPO_PATH}`, official origin `mcboyle/BD`. Deployed tree and working tree may be same dir (`test5`).
...
Deferred work = machine-visible row in canonical backlog (unique id, status, evidence, acceptance criteria, dependency). Prose like "later" not a deferral. No row for completed, obsolete, already-represented work. Any train may carry register commit (swarm-register-append/close/amend); workers PROPOSE row text, never assign ids -- integrator assigns. Register rows never gate runtime lane.
...
TRAIN = second sanctioned shape: up to sixteen independently reviewed patches, DISJOINT authored paths, one version trio, one lane, one exact-head CI. Width = throughput lever, since trio serializes landings. Lane failure bisected by patch with `swarm-train.sh --bisect`; offending patch returns to worker with lane log. Two patches sharing path ride different trains. Train changelog entry names every patch.
...
6. Run focused GREEN, negative/adversarial controls, complete affected floor with real pytest. T0/T1 run affected band, `swarm-precut --gate` tree gates, `swarm-freshcheck --repo-only`; no full canonical suite per cut.
...
9. Adversarial review per `project-knowledge/CUT_TIERING.md`:
   ...
   dispatcher checks worker's recorded `swarm-mutate` battery.
...
Some gates judge TREE, not diff, so `swarm-band-derive` can never select them. Run `toolchain/bin/swarm-precut --gate` before freezing; runs that undertow, refuses on failure. Green band necessary, not sufficient: adversarial review, schedule interactions, generated state, packaging, full suite = separate questions.
...
Before reporting `VERDICT: PATCH` on T2/T3 patch, worker runs correctness lens checklist on itself: RED command with EXACT failure text, GREEN command + result, negative control, exact-count assertion, swarm-mutate result, both tree-gate lines. DONE.md missing any = bounced by review dispatcher before lens spent. `swarm-review-prep` checks only DONE.md exists, line 1 exactly `VERDICT: PATCH`; floor enforced by dispatcher, not tool. T0/T1 patch owes only what tier owes.

T0/T1 cuts run affected band from `swarm-band-derive`, tree gates via `swarm-precut --gate`, `swarm-freshcheck --repo-only`; no full canonical suite per cut. Canonical suite runs once on DEPLOYED tree, per A6.

Use real pytest via repo interpreter. Derive affected tests with `toolchain/bin/swarm-band-derive`; output = floor, never ceiling -- add tree-wide denominators, deleted-file consumers, docs/freshness, generated, release, adversarial tests subject requires. Docs or backlog edit: also run:

```bash
venv/bin/python toolchain/bin/swarm-freshcheck --repo-only
```

Focused/affected pytest: remove ambient install-directory state:

```bash
env -u SWARM_INSTALL_DIR bash -c 'SWARM_DISABLE_KEEPALIVE=1 venv/bin/python -m pytest tests/test_target.py -q'
```

The only sanctioned canonical local full-suite command is:

```bash
env -u SWARM_INSTALL_DIR SWARM_DISABLE_KEEPALIVE=1 PYTHONUNBUFFERED=1 venv/bin/python -m pytest tests/ -n 24 --dist loadfile --timeout=240 --timeout-method=signal --max-worker-restart=0 -p no:randomly
```
...
Different worker count, scheduler, plugin, interpreter, env = different experiment, cannot authorize merge. Never export `SWARM_INSTALL_DIR` into pytest (`SWARM_HOME` is a different resource); pop with `env -u`.
...
Integrator stamps release trio at land with `toolchain/bin/swarm-land-trio`. Worker patches and `DONE.md` never carry trio or `PIN_INDEX.json` edits; assembler refuses worker patch touching those paths. Version bump = three source edits together: `bulk_downloader/__init__.py` sets `__version__`; `tests/test_settings_center_slice4.py` pins exact value; ASCII-only `CHANGELOG.md` entry prepended, anchored on previous release header. Then regenerate + inspect `PIN_INDEX.json`; don't assume pin count/location. Run version, changelog, generated, release, frontend, packaging gates against final candidate.
...
RENDERED PAGE IS EVIDENCE; CANDIDATE LIST IS CLAIM ABOUT IT. BD once saved 5 GB of wrong scene under right title because Related Videos grid exposed 159 media links; one screenshot answered it, same screenshot later proved three "mis-filed" rows correct. Capture what browser sees before theorising. Operator harness carries `swarm-shoot.py` (site id, URL, destination) for this; drives real browser against authenticated site, so operator instrument, never gate.
...
| Which tests does a changed path require? | `project-knowledge/TOUCHED_FILE_TO_TEST.md` and `toolchain/bin/swarm-band-derive` |
...
| What are the guarded files? | root `guards.json` and `toolchain/bin/swarm-guardcheck` |
| What safety declarations exist? | root `FOOTGUNS.json` and `INVARIANTS.json` |
| How are generated artifacts ordered? | `toolchain/bin/swarm-regen-order` |
| How is affected scope derived? | `toolchain/bin/swarm-band-derive` |
| How are mutations run safely? | `toolchain/bin/swarm-mutate` |
...
Before broad scan, name population, use `rg`/`git ls-files` or purpose-built tool. CAPTURE WHOLE TO DISK, READ A SLICE. A SECOND HAND-ROLLED HEREDOC IS A MISSING `swarm-*` TOOL. And measure before optimising: use complete captured denominator, then inspect bounded slices, promote repeated logic into toolchain. Generated remote source = transport data: send UTF-8 bytes through ASCII-safe decoder, verify digest before publication; never embed in bootstrap heredoc; test delimiter-collision payload + decodable-corruption refusal at real transport seam. Parallel read-only discovery fine when authorized, but one integrator, one writer remain. Local-model or worker classifications = proposals, never tests, reviews, absence proofs, merge approval, deployment authority.
