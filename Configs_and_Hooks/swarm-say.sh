#!/bin/bash
# swarm-say.sh <tmux-session> <text> -- type text into a claude pane and press Enter in ONE call (M35/M46),
# then prove it was consumed. REFUSES (nonzero) on any failed step; never fail-open.
set -u
S=$1; shift; if [ "${1:-}" = "-" ]; then T=$(cat); else T="$*"; fi
# ---- E6 FIX (swarm-pm-b 2026-09-07 19:3xZ). T0 IS THE SENDER'S ORIGINAL TEXT, CAPTURED BEFORE ANY
...
# STOP was refused at 520 chars, or (with SWARM_SAY_NOSPOOL=1) did not arrive at all. Bisected across
# four generations by swarm-review-a; the 18:50 backup is the positive control that proves it was alive
...
# ---- S1 (swarm-fable 04:3xZ, H72): ledger on every exit, broadcast limiter, opt-in composer guard.
...
LOG=${SWARM_SAY_LOG:-${SWARM_PERSIST_DIR}/say.log}; NOW=$(date +%s); SHA=pending
INBOX_ROOT=${SWARM_SAY_INBOX_ROOT:-${SWARM_SAY_INBOX:-${SWARM_PERSIST_DIR}/inbox}}   # H516: every inbox site files under one root (tests point it at scratch)
SAY_MODE=${SWARM_SAY_MODE:-type}
...
mailbox_for() {
  local pm role
  pm=$(grep -v '^#' ${SWARM_PERSIST_DIR}/PM-SEAT 2>/dev/null | tr '\n' ' ')
  case " $pm " in *" $1 "*) echo PM; return 0 ;; esac
  case "$1" in
    pm|PM) echo PM; return 0 ;;
    adjudicator|integrator|usage) echo "$1"; return 0 ;;
  esac
  if [ -f ${SWARM_PERSIST_DIR}/ROLE-OCCUPANCY.tsv ]; then
    role=$(awk -v s="$1" '$2==s {print $1; exit}' ${SWARM_PERSIST_DIR}/ROLE-OCCUPANCY.tsv 2>/dev/null)
    case "$role" in
      pm) echo PM; return 0 ;;
...
# ---- S7 (swarm-pm-B 2026-09-14 00:5xZ, rule 34): THE ROUTINE SINK MAY HAVE NO SESSION. FLEET_RULE 40 routes
# every routine line to swarm-status; under O341 the watchers are crons and swarm-status has had no tmux session
# since 2026-09-10, so 19 routine messages (say.log rc=3, "unknown-sender -> swarm-status") were LOST with only a
# ledger row -- the ledger even lacked the sender because FROM is derived below. A message to THE SINK is
# now FILED to inbox/swarm-status when the session is absent (SINK_FILE, handled beside PM_FILE after the
...
# has no exact session ("swarm-review-lite-B2:" resolves to swarm-review-lite-B2-B), so has-session said yes to
# the short name, the pane path typed into the -B seat, and the FILE-DROP paths below used the raw $S and
# wrote inbox/swarm-review-lite-B2 beside inbox/swarm-review-lite-B2-B -- a dir no seat drains. The target is
...
case "$S" in
  pm|PM)
    _resolved=$(cat ${SWARM_PERSIST_DIR}/PM-SEAT 2>/dev/null | grep -v '^#' | head -1 | tr -d '[:space:]')
    [ -z "$_resolved" ] && _resolved=$(/home/mboyle/swarm-role-claim.sh incumbent pm 2>/dev/null | tail -1 | tr -d '[:space:]')
    [ -n "$_resolved" ] && S="$_resolved"
    ;;
  adjudicator|integrator|usage)
    _resolved=$(/home/mboyle/swarm-role-claim.sh incumbent "$S" 2>/dev/null | tail -1 | tr -d '[:space:]')
    [ -n "$_resolved" ] && S="$_resolved"
    ;;
esac

# SWARM_SAY_TARGET / SWARM_SAY_PANE: redirect tmux pane targeting to a scratch/test session
bd_say_from(){
  local s map p pp i
  # O854 (2026-09-19): SWARM_SEAT is the launcher-exported seat identity and outranks every derivation.
  ...
  if [ -n "${SWARM_SEAT:-}" ]; then printf '%s' "$SWARM_SEAT"; return 0; fi
  if [ -n "${TMUX_PANE:-}" ]; then
  ...
  if [ -n "${SSH_CONNECTION:-}" ]; then printf 'ssh:%s' "${USER:-unknown}"
    ...
    _cn=$(tr '\0' ' ' < /proc/$PPID/cmdline 2>/dev/null | grep -oE 'swarm-[a-z0-9-]+\.(sh|py)' | head -1)
    if [ -n "$_cn" ]; then printf 'cron:%s' "${_cn%.*}"; else printf 'nontty:pid%s' "$PPID"; fi
  fi
}
PANE_TARGET=${SWARM_SAY_TARGET:-${SWARM_SAY_PANE:-$S}}
...
# (swarm-integrator-A 136, swarm-pm-F51-B 15, swarm-pm-O5-A 11), and live seats were still addressing those
...
if [ "${SWARM_SAY_REROUTE:-1}" = 1 ] && ! tmux has-session -t "=$PANE_TARGET" 2>/dev/null; then
  _role=$(printf '%s' "$PANE_TARGET" | sed -n 's/^swarm-\(pm\|integrator\|trainer\|triage\|status\|usage\|adjudicator\|registrar\|dispatch\|nurse\|audit\|scribe\|cartograph\)[-_].*/\1/p')
  _live=""
  if [ "$_role" = pm ]; then
    # PM-SEAT is append-ordered, so the LAST live entry is the incumbent; the claim is the tie-break.
    for _c in $(tac ${SWARM_PERSIST_DIR}/PM-SEAT 2>/dev/null | grep -v '^#'); do
      tmux has-session -t "=$_c" 2>/dev/null && { _live=$_c; break; }
    done
  fi
  if [ -z "$_live" ] && [ -n "$_role" ] && [ -x /home/mboyle/swarm-role-claim.sh ]; then
    for _c in $(/home/mboyle/swarm-role-claim.sh who "$_role" 2>/dev/null); do
      tmux has-session -t "=$_c" 2>/dev/null && { _live=$_c; break; }
    done
  fi
  if [ -n "$_live" ] && [ "$_live" != "$PANE_TARGET" ]; then
    printf '%s\t%s\t%s\t%s\n' "$(date -u +%FT%TZ)" "$PANE_TARGET" "$_live" "${_role:-?}" \
      >> ${SWARM_PERSIST_DIR}/logs/say-reroute.log 2>/dev/null || true
    ...
    IS_PM=0
    if grep -qFx "$PANE_TARGET" ${SWARM_PERSIST_DIR}/PM-SEAT 2>/dev/null; then IS_PM=1; fi
    IS_INT=0
    if [[ -x /home/mboyle/swarm-role-claim.sh && "$(/home/mboyle/swarm-role-claim.sh who integrator 2>/dev/null)" == "$PANE_TARGET" ]]; then IS_INT=1; fi

    if [[ $IS_PM -eq 1 || $IS_INT -eq 1 ]]; then
    if [[ "$T" == ESC* || "$T" == HIGH* || "$T" == REFUSED* || "$T" == LIMIT* || "$T" == STALL* || "$T" == BLOCKED* || "$T" == OUTAGE* || "$T" == CRASH* || "$T" == ORDER* || "$T" == POLL* || "$T" == *" POLL "* || "$T" == *" HIGH "* || "$_SND" == "operator" || "$_SND" == swarm-pm-* ]]; then
        echo "swarm-say: ESCALATION - bypassing batch queue for $PANE_TARGET"
    else
        export PYTHONPATH=/home/mboyle/teamwork_projects/lifecycle_optimizer
        if /home/mboyle/bin/swarm-batch-relay enqueue --target "$PANE_TARGET" --batch-size 10 --message "$T"; then
            echo "swarm-say: intercepted and batched via swarm-batch-relay to $PANE_TARGET"
            exit 0
        fi
        echo "swarm-say: relay daemon dead or rejected, falling through..."
    fi
fi
...
if [ "$PANE_TARGET" = "none" ] || [ "$PANE_TARGET" = "discard" ]; then
  ...
  if [ "$S" = "${SWARM_SAY_SINK:-swarm-status}" ] && [ "${SWARM_SAY_KIND:-}" != bootstrap ]; then SINK_FILE=1
  elif [ "${SWARM_SAY_FILE_FIRST:-0}" = 1 ]; then
    ...
    die 3 "swarm-say: NO SESSION $PANE_TARGET${NEAR:+ -- not a seat name; live seats matching it: $NEAR(use the exact name)}"
  fi
fi
...
case "${SWARM_SAY_KIND:-}" in bootstrap) : ;; *)
  if [ "${SWARM_SAY_FILE_FIRST:-0}" = 1 ]; then
    ...
    # swarm-persist/PM-SEAT (one session name per line; default BD-PM). `incumbent fable` had NO live
    # holder, so this filter never fired and every ACK reached the operator's window as a full turn.
    # ---- H471 (swarm-pm-A 2026-09-14, O585): the escalation vocabulary is now ONE regex, shared by
    ...
    # ---- H476 (O596(1), swarm-pm-A2-A 2026-09-14): the PM pane is the operator's window; only a WAKE word
    # types there. DONE/LANDED/REFUTE/CLAIM/DISPATCH/ESC/STOP are routing words for WORKERS (a DISPATCH
    # must still type to a worker) and are FILED for the PM (drained by swarm-inbox-drain). One vocabulary,
    ...
    # ("I have/I will/Let me/Note that/please", two or more sentences). Ledger: swarm-persist/state/prose-violations.tsv.
    # Two violations by one seat = a pattern (FLEET_RULE 34) -> swarm-prose-enforce.sh orders a handoff and retires the seat.
    _PF=$(bd_say_from 2>/dev/null || echo unknown)
    case "$_PF" in swarm-*-Z|swarm-*-Z[0-9]*) : ;; swarm-*)   # -Z seats are test fixtures (H516 probe), never measured
      _PT=$(printf '%s' "$T0" | sed -E 's#(/[A-Za-z0-9_./-]+)##g; s#\[from [^]]*\] ##; s#https?://[^ ]+##g')
      # Harness-generated lines are not prose: swarm-dispatch.sh "START NOW:" (H334 imperative), assign-lens "NEXT: claim",
      ...
      if [ -n "$_PV" ]; then
        mkdir -p ${SWARM_PERSIST_DIR}/state
        printf '%s\t%s\t%s\t%s\n' "$(date -u +%FT%TZ)" "$_PF" "$_PV" "$(printf '%s' "$_PT" | cut -c1-70)" >> ${SWARM_PERSIST_DIR}/state/prose-violations.tsv
      fi ;;
    esac
    PM_SEAT=$(cat ${SWARM_PERSIST_DIR}/PM-SEAT 2>/dev/null | grep -v '^#' | tr '\n' ' '); PM_SEAT=${PM_SEAT:-BD-PM}
    # ---- O670 (operator 2026-09-15 02:0xZ): a TRIAGE seat sits in front of the PM. Agent traffic
    # addressed to the PM is RETARGETED to the triage seat named in swarm-persist/TRIAGE-SEAT when that
    # seat is live; the operator (ssh/interactive), the triage seat itself, cron:swarm-pm-heartbeat and
    ...
    TRIAGE_SEAT=$(grep -v '^#' ${SWARM_PERSIST_DIR}/TRIAGE-SEAT 2>/dev/null | head -1)
    ...
    if [ -n "$TRIAGE_SEAT" ] && printf ' %s ' $PM_SEAT | grep -qF " $S " && tmux has-session -t "=$TRIAGE_SEAT" 2>/dev/null \
      ...
      case "$_F" in ssh:*|operator*|"$TRIAGE_SEAT"|cron:swarm-pm-heartbeat|cron:swarm-scout-budget|cron:swarm-triage*) : ;;
        *) if printf ' %s ' $PM_SEAT | grep -qF " $_F "; then : # O752: a PM seat (incumbent<->successor) reaches the PM pane, never triage
           else echo "swarm-say: PM traffic from $_F retargeted to triage seat $TRIAGE_SEAT (O670)"; S=$TRIAGE_SEAT; fi ;; esac
    fi
    if [ -n "$PM_SEAT" ] && printf ' %s ' $PM_SEAT | grep -qF " $S "; then
      ...
      # swarm-persist/PM-MUTE: one sender per line whose traffic is ALWAYS filed (a seat that loops on the PM pane).
      _MUTED=""; [ -f ${SWARM_PERSIST_DIR}/PM-MUTE ] && grep -qxF "$_F2" ${SWARM_PERSIST_DIR}/PM-MUTE && _MUTED=1
      if [ -z "$_MUTED" ] && printf '%s' "$T0" | grep -qiE "$PM_WAKE_RE" && ! printf '%s' "$_F2" | grep -qE '^(nontty:|unknown)' \
        ...
        if printf '%s' "$T0" | grep -qiE '\b(HIGH|BLOCKED|BLOCKER)\b'; then
          ${SWARM_PERSIST_DIR}/harness/swarm-push-alert.sh "$T0" 2>/dev/null || true
        fi
      else
        ...
        # swarm-say targets `$S:` which resolves to the ACTIVE PANE (proven: a split session answered
        ...
        # CAN NO LONGER REFUSE A REPORT (rc=6, FLEET_RULE 33 -- swarm-lens-b3's BOARD, refused twice).
        # STOP/LIMIT and bootstraps are already exempt above and still reach the pane.
        if [ "${SWARM_SAY_PM_INBOX:-1}" = 1 ]; then
          # FILE ONLY, TYPE NOTHING. The old spool still typed an "INBOX <path>" pointer, which is
          # a full turn for the PM (its whole cached context re-read). swarm-pm-heartbeat.sh drains
          # the inbox as ONE line per tick. The write happens after the identity stamp (PM_FILE).
          PM_FILE=1
        fi
      fi
    elif [ "${SWARM_SAY_BATCH:-1}" = 1 ] && [ "${SWARM_SAY_MODE:-}" != "board" ]; then
      ...
      # lite-B 974 in 12h, most routine ACK/relay). Codex seats are named swarm-cx-* fleet-wide (same
      # test swarm-dispatch-check.sh uses) and are explicitly OUT of scope -- they have no inbox-drain
      # cadence and nothing here changes them. SWARM_SAY_BATCH=0 lets swarm-inbox-drain.sh's own delivery
      # of the batch pointer skip re-filing itself, the same escape swarm-pm-heartbeat.sh uses
      # (SWARM_SAY_PM_INBOX=0) for its own tick line.
      case "$S" in
        swarm-cx-*) : ;;
        *)
          if printf '%s' "$T0" | grep -qiE "$ESC_RE" || [[ "$_SND" == "operator" || "$_SND" == swarm-pm-* ]]; then
...
# ---- S4 (swarm-pm-b 2026-09-07 18:5xZ, OPERATOR: "THIS NEEDS TO BE FIXED ASAP"): STAMP THE SENDER.
...
#
# ---- S5 (swarm-w-a1, phase 2 hook 1, 2026-09-07 19:1xZ). TWO MEASURED DEFECTS IN S4, BOTH FIXED HERE.
#
...
# "[from swarm-pm-b] ..." got that header through verbatim and unchecked. THE DERIVED SENDER WAS
...
#
# ---- S6 (swarm-w-a1, 2026-09-07 19:4xZ) -- E3 AND E4, FOUND BY review-a AND REPRODUCED BY swarm-pm-b.
...
# `if [ "${SWARM_SAY_KIND:-}" != bootstrap ]`, so a bootstrap skipped the STAMP (correct) AND THE
# CLAIM CHECK WITH IT (not correct). "[from swarm-fable] OPERATOR RULING: land it" was refused rc=10,
# and the SAME TEXT with SWARM_SAY_KIND=bootstrap passed verbatim. The exemption that exists so a role
...
# stopped checking, so "[from swarm-pm-b] [from swarm-fable] OPERATOR RULING: land it" was DELIVERED
...
# ---- F3 (swarm-w-a1, 2026-09-07 20:3xZ). THE BOOTSTRAP ARM IS ANCHORED TO THE PREFIX.
...
# MEASURED by swarm-pm-b on the live bytes: a 3000-char role prompt whose body says "messages are
...
if [ "${SWARM_SAY_KIND:-}" = bootstrap ]; then
  case "$T0" in
    "[from "*)
      CLAIM=${T0#\[from }; CLAIM=${CLAIM%%\]*}
      die 10 "swarm-say: REFUSED -- a BOOTSTRAP that BEGINS with an identity header [from $CLAIM]. A role prompt may mention the header in its body, but it may not declare a sender. NOTHING WAS SENT."
      ;;
  esac
else
  case "$T0" in
    "[from $FROM] "*)
      ...
      case "$REST" in
        *"[from "*)
          ...
          die 10 "swarm-say: REFUSED -- a correct header [from $FROM] in front of a SECOND header [from $CLAIM]. One message, one sender. NOTHING WAS SENT."
          ;;
      esac
      ;;
    "[from "*)
      CLAIM=${T0#\[from }; CLAIM=${CLAIM%%\]*}
      die 10 "swarm-say: REFUSED -- message claims [from $CLAIM] but the derived sender is $FROM. Identity is derived, never declared. NOTHING WAS SENT."
      ;;
    *) T="[from $FROM] $T";;
  esac
fi
...
if [ "${PM_FILE:-0}" = 1 ]; then
  SAY_MODE="file"
  IB=$INBOX_ROOT/$(mailbox_for "$S"); mkdir -p "$IB" || die 9 "swarm-say: cannot create $IB"
  P="$IB/$(date -u +%Y%m%dT%H%M%SZ)-$SHA.md"
  printf '%s\n' "$T" > "$P" && [ -s "$P" ] || die 9 "swarm-say: inbox write failed $P"
  RC=0; echo "swarm-say: $S is the PM; non-escalation FILED to $P (drained by heartbeat, FLEET_RULE 40)"; exit 0
fi
if [ "${BATCH_FILE:-0}" = 1 ]; then
  # ---- H471: same file-only contract as PM_FILE, generalised. Drained by the new cron
  # swarm-inbox-drain.sh (*/5) as one batched swarm-say per seat, mirroring swarm-pm-heartbeat.sh's INBOX
  # batch (BATCH-<ts>.md concatenation + single pointer line), never re-filed (SWARM_SAY_BATCH=0).
  SAY_MODE="file"
  IB=$INBOX_ROOT/$(mailbox_for "$S"); mkdir -p "$IB" || die 9 "swarm-say: cannot create $IB"
  P="$IB/$(date -u +%Y%m%dT%H%M%SZ)-$SHA.md"
  printf '%s\n' "$T" > "$P" && [ -s "$P" ] || die 9 "swarm-say: inbox write failed $P"
  RC=0; echo "swarm-say: $S non-escalation FILED to $P (drained by swarm-inbox-drain, H471/FLEET_RULE 40)"; exit 0
fi
if [ "${SINK_FILE:-0}" = 1 ]; then
  SAY_MODE="file"
  IB=$INBOX_ROOT/$(mailbox_for "$S"); mkdir -p "$IB" || die 3 "swarm-say: NO SESSION $S and cannot create $IB"
  P="$IB/$(date -u +%Y%m%dT%H%M%SZ)-$SHA.md"
  printf '%s\n' "$T" > "$P" && [ -s "$P" ] || die 3 "swarm-say: NO SESSION $S and inbox write failed $P"
  RC=0; echo "swarm-say: NO SESSION $S (routine sink); FILED to $P (S7, FLEET_RULE 40)"; exit 0
fi
# ---- S2 (swarm-pm-b 2026-09-07 05:5xZ): INTERIM LENGTH GUARD. THE ~30-CHAR RULE IS OPERATOR LAW.
...
# THE DETAIL GOES IN A FILE AND THE MESSAGE CARRIES THE PATH. SWARM_SAY_LONG=1 overrides for a genuine
# payload, and the override is visible afterwards because the ledger records the exit code.
# ---- F4 (swarm-w-a2 21:2xZ, applied by swarm-fable 2026-09-08 on the operator's order).
...
# SEAT-DEPENDENT: swarm-w-a2 was refused at 385 real chars, swarm-integrator-a3 at 376. Nobody chose that.
...
# ---- S3 (swarm-pm-b 2026-09-07 11:5xZ, operator-approved): AUTO-SPOOL TO A PER-SEAT INBOX.
    ...
    if [ "${SWARM_SAY_FILE_FIRST:-0}" = 1 ]; then
      ...
      if mkdir -p "$IB" 2>/dev/null; then
        ...
        if printf '%s\n' "$T" > "$P" 2>/dev/null && [ -s "$P" ]; then
          ...
          if [ "${SINK_FILE:-0}" = 1 ]; then
            RC=0; echo "swarm-say: FILED to $P (SWARM_SAY_FILE_FIRST)"; exit 0
          fi
        fi
      fi
    elif [ "${SWARM_SAY_NOSPOOL:-0}" != 1 ] && [ "$LEN" -gt "${SWARM_SAY_SPOOL_AT:-120}" ]; then
      ...
      if mkdir -p "$IB" 2>/dev/null; then
        P="$IB/$(date -u +%Y%m%dT%H%M%SZ)-$SHA.md"
        if printf '%s\n' "$T" > "$P" 2>/dev/null && [ -s "$P" ]; then
          # ---- F2 (swarm-w-a2 21:2xZ, applied by swarm-fable 2026-09-08 on the operator's order).
...
case "$T0" in STOP*|LIMIT*) ;; *)
  ...
  case "$S" in cx-*|*codex*)
    if [ "$LEN" -gt 1000 ]; then
      die 9 "swarm-say: REFUSED -- $LEN chars to Codex session $S: courier over 1KB collapses in Codex pane without delivery (H113). Write detail to a file and send the PATH."
    fi
  ;; esac
  # A ROLE BOOTSTRAP IS A LEGITIMATE LONG PAYLOAD AND MY GUARD BROKE EVERY LAUNCH.
  # swarm-launch-role.sh pipes a ~3000-char role prompt through swarm-say; from 05:5xZ this guard
  # refused it, and the sessions came up UNBOOTSTRAPPED -- which is exactly what happened to
  # swarm-worker-b5 and b6 at 06:11Z, and why they sat idle for four and a half hours.
  if [ "${SWARM_SAY_LONG:-0}" != 1 ] && [ "${SWARM_SAY_KIND:-}" != bootstrap ] && [ "$LEN" -gt 400 ]; then
    die 9 "swarm-say: REFUSED -- $LEN chars, the rule is ~30. Write the detail to a file and send the PATH. NOTHING WAS SENT. SWARM_SAY_LONG=1 overrides."
  fi
  if [ "$LEN" -gt 120 ]; then echo "swarm-say: WARN -- $LEN chars, the rule is ~30 (operator law). Put the detail in a file and send the path." >&2; fi
;; esac
# ---- S7 (swarm-w-a2, phase 2 hook 2, 2026-09-07 19:4xZ): THE LIMITER'S awk WAS NEVER SHIFTED WHEN S4
...
case "$T0" in STOP*|LIMIT*) ;; *) if [ "${SWARM_SAY_KIND:-}" != bootstrap ] && [ -f "$LOG" ]; then
  ...
  [ "${n:-0}" -ge 3 ] && die 8 "swarm-say: REFUSED -- broadcast limiter: this text already went to $n targets in 5 min. Rules go to OPERATOR_DECISIONS.md; swarm-say only the session whose next action changes."; fi;; esac
# ---- REFUSE A PANE THAT IS NOT AN INPUT BOX. This tool types text and then
# presses Enter UNCONDITIONALLY. On 2026-09-05 at 03:1xZ swarm-fable was not at an
...
if [ "${SWARM_SAY_STRICT:-0}" = 1 ]; then LIVE=$(printf '%s\n' "$PANE0" | grep '^❯' | tail -1)
  { printf '%s\n' "$LIVE" | grep -q '^❯ [^[:space:]]' || printf '%s\n' "$TAIL15" | grep -q 'esc to interrupt'; } && die 7 "swarm-say: REFUSED (strict) -- $S composer busy or carrying text. Nothing was sent."; fi
...
# still caught. SWARM_SAY_MENU_STRICT=1 restores the count-only refusal.
CURSORED=$(printf '%s\n' "$TAIL15" | grep -cE '^[[:space:]]*[>â¯][[:space:]]*[0-9]+\.[[:space:]]+[^[:space:]]')
...
# swarm-pm-A2-A, b01f6f2c x5 and cb433a31 x6 to swarm-pm-F2-A) and the text never arrives. The dialog blocks the PANE, not the
# inbox: the THIRD identical refusal for the same (sender, target, sha) is FILED to the target's inbox (drained by
# swarm-inbox-drain / the PM heartbeat, FLEET_RULE 40) and exits 0 with a "do not retry" line. Two refusals still exit 6.
refuse6(){
  ...
  if [ "${n:-0}" -ge 2 ]; then
    ...
    if mkdir -p "$IB" 2>/dev/null && printf '%s\n' "$T" > "$P" && [ -s "$P" ]; then
      RC=0; echo "swarm-say: $S still holds a dialog (rc=6 already $n x for $SHA); FILED to $P (H516) -- DO NOT RETRY"; exit 0
    fi
  fi
  echo "$1"; RC=6; exit 6
}
if [ "${NOPT:-0}" -ge 2 ] && { [ "${CURSORED:-0}" -ge 1 ] || [ "${CONFIRM:-0}" -ge 1 ] || [ "${SWARM_SAY_MENU_STRICT:-0}" = 1 ]; }; then
  refuse6 "swarm-say: REFUSED -- $S is showing a $NOPT-option SELECTION MENU (cursored=$CURSORED confirm=$CONFIRM), not an input box. Typing here presses Enter on someone else's choice. Nothing was sent."
fi
if printf '%s\n' "$TAIL15" | grep -qiE 'ready to submit your answers|do you want to (proceed|continue)|press enter to (confirm|continue)|\(y/n\)'; then
  refuse6 "swarm-say: REFUSED -- $S is showing a CONFIRM PROMPT, not an input box. Nothing was sent."
fi
# ---- F1a + F1b (swarm-w-a1, 2026-09-07 21:1xZ). ** THE DELIVERY PROBE IS NOW ARRIVAL-BASED. **
...
# matched ITS OWN EARLIER COPY. swarm-say's one guarantee -- type it and PROVE it was consumed --
...
PRE=$(tmux capture-pane -e -pt "$PANE_TARGET:" -S - | _o945_strip_ghost; exit "${PIPESTATUS[0]}") || die 3 "swarm-say: could not capture $PANE_TARGET BEFORE sending -- arrival would be unmeasurable. NOTHING WAS SENT."
tmux send-keys -t "$PANE_TARGET:" -l "$T" || die 4 "swarm-say: send failed"
...
if [ "${SWARM_SAY_QUICK_TEST:-0}" != 1 ]; then
...
tmux send-keys -t "$PANE_TARGET:" Enter || die 4 "swarm-say: enter failed"
[ "${SWARM_SAY_QUICK_TEST:-0}" != 1 ] && sleep 5
...
if [ -n "$LAST" ] && { printf '%s\n' "$LAST" | grep -qE '^(❯|›|>) .{20,}' || printf '%s\n' "$LAST" | grep -qE '\[Pasted Content [0-9]+ chars\]'; }; then
  ...
  if printf '%s\n' "$STATUS_TAIL" | grep -qiE '[0-9]+[[:space:]]+monitor|\bmonitors\b|/loop|\bloop\b'; then
    ...
    # H371: swarm-say send-keys can leave text in target input line unsubmitted.
    # When text matches our needle (or collapsed paste payload), send Enter again and log RESENT-ENTER.
    echo "swarm-say: $S RESENT-ENTER (H371 unsubmitted text retry)" >&2
    printf '%s\t%s\t%s\tRESENT-ENTER\t%s\t%s\t%s\n' "$(date -u +%FT%TZ)" "${FROM:-unknown-sender}" "$S" "${#T}" "$SHA" "$(printf '%s' "$T" | tr '\n\t' '  ' | cut -c1-300)" >> "$LOG" 2>/dev/null
    tmux send-keys -t "$PANE_TARGET:" Enter || die 4 "swarm-say: enter retry failed"
    sleep 3
    P=$(tmux capture-pane -e -pt "$PANE_TARGET:" -S - | _o945_strip_ghost)
    LAST=$(printf '%s\n' "$P" | grep -E '^(❯|›|>)' | tail -1)
    if [ -n "$LAST" ] && { printf '%s\n' "$LAST" | tr -d '[:space:]' | grep -qF -- "$NEEDLE" || printf '%s\n' "$LAST" | grep -qE '\[Pasted Content [0-9]+ chars\]'; }; then
      STATUS_SUMM=$(printf '%s\n' "$P" | tail -6 | tr '\n' ' ' | cut -c1-80)
      die 2 "swarm-say: $S STILL IN INPUT BOX after RESENT-ENTER -- UNKNOWN (status: $STATUS_SUMM); inspect status line"
    fi
  fi
fi
# 2026-09-07 07:4xZ, swarm-dispatch: A WRAPPED MESSAGE IS NOT AN UNDELIVERED ONE.
# This probe grepped "${T:0:40}" as ONE CONTIGUOUS LINE. tmux WRAPS at the pane
# width, so a 130-char send to swarm-lens-b3 was split across two rendered lines,
...
# swarm-pm-b read it and trusted it; so did I. A JUSTIFICATION THAT NO LONGER HOLDS IS WORSE THAN
...
for _try in 1 2 3; do
  ...
  if [ -n "$LAST_LIVE" ] && printf '%s\n' "$LAST_LIVE" | grep -qE '\[Pasted Content [0-9]+ chars\]|\bPasted Content\b'; then
    die 5 "swarm-say: $S COLLAPSED PASTE in composer ([Pasted Content ...]) -- courier collapsed in composer without submission (H113/H132). Nothing was submitted. Write detail to file and send the path."
  fi
  [ "$SEEN" -eq 1 ] || die 5 "swarm-say: $S text NOT SEEN IN THE LINES THAT ARRIVED after 3 looks -- UNKNOWN. The pane may already hold an older copy; that is NOT delivery of THIS send."
RC=0; echo "swarm-say: $S sent+consumed"
